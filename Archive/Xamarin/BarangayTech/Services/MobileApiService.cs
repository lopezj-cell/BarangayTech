...
namespace BarangayTech.Services
{
    public static class MobileApiService
    {
        ...
        // --- AI Endpoints ---
        // Input / output DTOs
        public class TextInput { public string Text { get; set; } }
        public class TranslateInput { public string Text { get; set; } public string TargetLang { get; set; } }
        public class FeedbackOutput { public string Sentiment { get; set; } public string[] Topics { get; set; } }
        public class SuggestTaskInput { public string Title { get; set; } public string Description { get; set; } }
        public class SuggestTaskOutput { public string SuggestedPriority { get; set; } public string SuggestedCategory { get; set; } }
        ...
        public static async Task<string> SummarizeAsync(string text, CancellationToken ct = default)
        {
            try
            {
                ...
                if (resp.TryGetProperty("result", out var r) && r.ValueKind == JsonValueKind.String) return r.GetString();
                if (resp.TryGetProperty("output", out var o) && o.ValueKind == JsonValueKind.String) return o.GetString();
        ...
        public static async Task<string> TranslateAsync(string text, string lang, CancellationToken ct = default)
        {
            try
            {
                ...
                if (resp.ValueKind == JsonValueKind.Object && resp.TryGetProperty("translated", out var p) && p.ValueKind == JsonValueKind.String)
                    return p.GetString();
                // fallback checks
                if (resp.TryGetProperty("translation", out var t) && t.ValueKind == JsonValueKind.String) return t.GetString();
                return resp.ToString();
            }
            catch
            {
        ...
        public static async Task<FeedbackOutput> AnalyzeFeedbackAsync(string text, CancellationToken ct = default)
        {
            ...
            {
                return new FeedbackOutput { Sentiment = string.Empty, Topics = new string[0] };
            }
        }
